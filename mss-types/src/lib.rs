...
//!
//! Corona **leaf 7**, and the garden's first **composition leaf**. Leaves 1–6 each
//! demonstrate a primitive *in isolation*; with leaf 6 the vocabulary (E0451 /
...
//!   chain is not `Clone`, so signing from a stale chain state — the classic
//!   *hash-based-signature state-reuse catastrophe* — does not compile. Inside,
...
//!   *that is a committed member of this root*.
...
//!   facts (the leaf index, the digest) escape into [`VerifiedMssMessage`]. The
//!   brand does exactly what it does at home: witnesses stay bound to the check
//!   that minted them.
...
//!   stale-state hazard is a compile error, and dropping the chain forfeits its
//!   remaining capacity (affine, at-most-*n* — deliberately, as in leaf 5).
...
/// `n` signatures.
///
/// Sealed (E0451: minted only by [`generate`]) so that in-process provenance is
/// honest, but — like `merkle-types`' `Root`, whose data this is — it is a
/// **caller-trusted anchor**, not self-certifying: verification proves validity
/// under *this* root, never that this root is the right signer's.
...
/// consumes the chain state that produced it: signing from a stale state is a
/// compile error (E0382). `Debug` redacts, mirroring the inner key's own policy.
///
...
/// `0xFF` — outside the `{0, 1}` sides keygen itself uses, so chain-level and
/// key-level derivations cannot collide. (Deterministic derivation is the toy
/// choice, for reproducible tests; it is also exactly what the "per chain value,
/// not per chain material" honest limit is about.)
...
impl MssKeychain {
    ...
    ///
    /// Taking `self` by value is the composition's E0382 at work: a chain state
    /// signs at most once, so the stale-state reuse that plagues stateful
    /// hash-based signatures is a compile error, not an operational hazard.
    /// Internally the popped [`SigningKey`] is itself consumed by leaf 5's
    /// `sign(self, …)` — linearity threaded, not reimplemented.
...
impl MssPublicKey {
    ...
    pub fn verify(&self, message: &[u8], sig: &MssSignature) -> Option<VerifiedMssMessage> {
        ...
        // `size >= 1` by construction, so adoption itself never refuses; the
        // flatten only merges the two Option layers (adoption, verification).
        .flatten()
    }
}
