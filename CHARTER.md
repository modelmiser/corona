...
| `ecash-types` | research (toy) | bearer value / double-spend prevention (negative space) | the first **negative-space leaf** — leaves 1–8 all answered *yes*; this one asks where the vocabulary **provably stops**, and answers with a three-layer **split**, each layer executable. **Layer 1 (one ownership graph): reduces to E0382** — `Coin` is not `Clone`/`Copy`, `into_wire(self)` consumes it, double-spend is a compile error (verified `error[E0382]`; leaf 5's consumable capability, applied to value; affine-not-linear is again the *right* direction — dropping a coin burns it, spending twice is the catastrophe). **Layer 2 (across the wire): does NOT reduce, definitionally** — a type discipline binds only the program it type-checks; a serialized coin is bytes outside every program, so `WireCoin` is honestly all-public + `Copy` (the doorway type witnesses *nothing*), a double spend *type-checks*, and prevention falls to the mint's **spent set** (`Mint::redeem`: runtime, stateful, online; tag checked *before* the set, so `DoubleSpent` implies authentic, forgery neither probes nor burns). The missing piece is **not a fifth compile primitive** — it is *fresh knowledge at redeem time*, which no compile-time fact (fixed before the adversary acts) can supply. **Layer 3 (replicated mint): the coordination seam** — the spent set is fused to one `Mint` *value*; two mints from one seed share identity (`minted_by` cannot distinguish them) but not spent sets, and one wire coin redeems at both (regression-tested). "Unspent" is knowledge about **absence** — non-monotone (CALM, Hellerstein–Alvaro) — so replicas must coordinate: `quorum-types`' witness species, out of Corona's scope *by thesis*. The leaf is the seam between the gardens, drawn from Corona's side. Pedigree agrees with the cut: Chaum 1982 = exactly layer 2 (online mint, spent list); Chaum–Fiat–Naor CRYPTO '88 (offline) does not prevent but *reveals identity* after the fact — punish, not prevent; hardware "prevention" just relocates the spent state into an uncopyable box. Standalone (imports nothing — a boundary-drawing leaf must not lean on the vocabulary it bounds); both witness species in one leaf (linear `Coin` vs clonable `Receipt`); redacting `Debug` on tag + mint secret. E0451+E0382 used; brand/E0080 honestly unused. TOY invertible FNV tag (one observed coin ⇒ forge-at-will); no blinding/denominations/transfer |
...
- **Leaf 9:** nothing to promote, by design twice over: a negative-space leaf
  must not lean on the vocabulary it is bounding (importing siblings would
  entangle the "no" with their surfaces), and nothing in it is shared — the toy
  MAC is leaf-local like every other toy backend. `ecash-types` joins
...
`ecash-types` (the negative-space leaf — the first "no", drawing the vocabulary's
outer edge at the wire and handing off to `quorum-types` where coordination
begins). Corona names the family these already form; it is recognition, not new
scope.

### Candidate future leaves

- *(None scheduled. The **vocabulary is complete** (leaf 6), **composition is
  demonstrated** (leaf 7 — MSS) and **repeated** (leaf 8 — VID), and the
  **outer edge is drawn** (leaf 9 — the e-cash negative-space leaf, the first
  "no"). The garden could wind down as a finished thought. Remaining candidates
  are pure breadth: threshold signatures with linear nonces (FROST-style — three
  primitives jointly); a key ratchet (forward secrecy as two-sided linearity);
  an accumulator with witness staleness; a fountain/LT code; XMSS-style tiered
  trees.)*
