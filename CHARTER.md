...
reduces, timeline-freshness does not, re-landing on leaf 9's wire). Corona names the
family these already form; it is recognition, not new scope.
...
- *(None scheduled. The **vocabulary is complete** (leaf 6), **composition is
  ...
  **first boundary point is drawn** (leaf 9 — the e-cash negative-space leaf,
  the first "no"), and **both value primitives are read to their widest with a
  ...
  accumulator, instance-identity vs timeline-freshness). The garden could wind
  down as a finished thought. Remaining candidates are pure breadth: threshold
  signatures with linear nonces (FROST-style — three primitives jointly); a
  fountain/LT code; XMSS-style tiered trees. (A key ratchet — done, leaf 10; an
  accumulator with witness staleness — done, leaf 11.))*
