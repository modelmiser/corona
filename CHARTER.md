...
Part 3 — `binding_reduces`/`binding_of_collisionFree` **discharge** binding as a reduction to the
backend's collision-resistance (bloom's face — meaningful only now the backend is vetted, not the FNV
triviality), while `fixed_blind_leaks` **exhibits** hiding's un-typability boundary (consttime's face).
Where wires 12 and 13 were the two faces on *separate* leaves, commit's graduation shows them on the
*same* subject, split by its own definition. (It is fan-in 0 **and** fan-out 0 — the cleanest possible
...
- **Leaf 9:** nothing to promote, by design twice over: a negative-space leaf
  ...
  `fnv1a` does textually recur in lamport (and did in merkle before it graduated to SHA-256), but toy backends are
...
- **Leaf 10:** nothing to promote (∥ leaves 4/5/9 — hash-based, single-chain,
  ...
  lamport/ecash FNV hashes (merkle's has since graduated to SHA-256), but toy backends are graduation-swap
