...
correct precedents are **pow and ecash** (load-bearing swaps over abstract-backend wires), *not*
bloom/translog, whose swaps were not load-bearing; what is unique to lamport is narrower — it is the only
wire that **pre-existed its own graduation**, so it is the only place the question "did the swap move
anything?" could be asked of theorems written beforehand. Invariance under a security-critical change is a
