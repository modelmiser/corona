...
>
> Criterion #5 is **recorded for exactly one of the ten `**graduated**` rows** —
> `accumulator-types`, whose round-by-round record in `TODO.md` shows seven rounds and no clean
> one. The other nine carry the label with **no recorded assessment against #5 at all**. So the
> leaf that tracked the criterion honestly is the only one visibly failing it, and the label's
> *operative* meaning across the registry has been "the toy backend is gone", not "all five hold".
>
...
rounds and rounds 1–7 have each been NOT CLEAN, so the registry row above marks the backend
...
numbers stopped being true — the decay class this document names elsewhere, live in the
document that adjudicates the graduation.)
