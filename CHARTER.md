...
computing the old key back from the new one — the backend's job). The toy FNV backend merely *abstained*
from the inversion guarantee (which the leaf declared out of scope); SHA-256's preimage resistance now
supplies it. So the swap is load-bearing in a **weaker** sense than pow's: pow's toy made the leaf's own
...
*named*, pow's sibling; naming SHA-256 in the injective branch is a **conservative worst case** — a
random-function SHA-256 step is generically *non*-injective, likely the discharged branch, but preimage
resistance is the operative assumption in both). The first wire whose residue is proved under one backend
