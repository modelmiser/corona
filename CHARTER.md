...
rounds and rounds 1–4 were each NOT CLEAN, so the registry row above marks the backend swap,
not a convergence claim; `TODO.md` carries the round-by-round record and is the referent.
What the arc is *for* is already clear even unconverged: **every round's CRITICAL was
introduced by the previous round's fix**, three times running, always in the same paragraph —
