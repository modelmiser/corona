...
| `arq-types` | research (toy) | reliable delivery over a lossy channel (stop-and-wait ARQ) | does **reliable delivery reduce?** → **it SPLITS along the safety/liveness line — the first leaf to cross it** (Lamport 1977; Alpern–Schneider 1985). Every prior residue is a **safety** fact (a violation has a *finite* witness); reliable delivery lands on **both sides at once**. *At-most-once, in-order delivery is SAFETY and reduces to E0451* — `Receiver::accept` is the sole minter of the sealed `Delivered` witness, minting one only for the in-order frame and re-acking every duplicate (the dedup a runtime sequence check ∥ leaf 1's count; the *witness a delivery happened* the seal). *"EVENTUALLY delivered" is LIVENESS and reduces to no primitive AND no finite check* — the identical protocol code delivers over a `FairChannel` (`Some`) and never over a `DeadChannel` (`None`, any bound), so **no fact about the code distinguishes them** (only the environment's *infinite* behaviour differs), and **no finite observation does either** (a channel carrying at round `N` is indistinguishable from one that never carries over the first `N−1` rounds — Alpern–Schneider's *no finite bad prefix*, made an executable test). A type is a compile fact, a runtime guard a finite check; **liveness is neither**, escaping deeper than any prior residue (not "a type can't but a runtime check can" — leaf 9/11 — but *nothing observable in finite time can hold it at all*). Discharged only by an **assumption about the environment** (the channel is *fair*, `□◇carries`) plus a **temporal argument over infinite runs** (`□◇carries ⟹ ◇delivered`) — the garden's **fourth seam** (leaf 9→coordination, 15→proof/Sol, 23→trust, **24→a fairness assumption + temporal reasoning**, the single-channel sibling of the **FLP impossibility**, Fischer–Lynch–Paterson 1985). **The doorway type inverts polarity:** a `Frame` is `Copy` like ecash's `WireCoin`/swap's `WireToken`, but copyability is the **cure** not the catastrophe (reliable delivery *retransmits copies* to beat loss) → **E0382 is structurally contra-indicated** (a linear frame forbids the remedy), the threat model flipped from *duplication* to *loss*. One primitive (E0451); brand/E0080 unused, E0382 contra-indicated; no new one. Standalone (∥ ecash/swap — a boundary-drawing leaf must not lean on siblings). TOY: frames unauthenticated (orthogonal — a dead channel still never delivers); ack path lossless; one payload, stop-and-wait, no windows/wraparound; single-byte payloads (keep the frame `Copy`) |
...
- **Leaf 18:** nothing to promote (standalone — imports neither core module nor a sibling
  ...
  *parameter* moves to a compile-time wall (`1≤BITS≤64`, ∥ `K≤N`) while the *effort* stays an
...
- **Leaf 24:** nothing to promote (standalone — imports neither core module nor a sibling leaf; needs no
  crypto backend at all ∥ leaf 23). What it adds is a **new residue *axis***: every prior residue, across
  all twenty-three, is a **safety** fact (a violation has a *finite* witness); `arq-types` is the first
  ...
  **liveness** → no primitive **and no finite check**: the identical code delivers over a fair channel and
  ...
  `□◇carries`) plus temporal reasoning over infinite runs** (`□◇carries ⟹ ◇delivered`) — the single-channel
  sibling of the **FLP impossibility** (Fischer–Lynch–Paterson 1985). Crucially, *no proof about our code*
  ...
  about a world the types do not touch. And the **doorway inverts polarity** — a `Copy` `Frame` is the
  *cure* (retransmit copies to beat loss), so **E0382 is structurally contra-indicated**, sharper than
  "honestly unused"; the threat model flipped from *duplication* (leaf 9/23) to *loss*. One primitive
  (E0451); brand/E0080 unused, E0382 contra-indicated; no new one.
...
`1≤BITS≤64` (E0080, the second E0451+E0080 leaf) while the *effort* stays runtime — and
...
(`□◇carries ⟹ ◇delivered`), the single-channel sibling of FLP; and the doorway type *inverts
polarity* — a `Copy` `Frame` is the **cure** (retransmit copies to beat loss), so E0382 is
structurally *contra-indicated*, the threat flipped from duplication to loss). Corona
...
- *(None scheduled. The **vocabulary is complete** (leaf 6), **composition is
  ...
  **Every named breadth candidate is built, and the two sibling gardens are both wired to
  ...
  finite bad prefix*) and the fourth seam (a fairness assumption + temporal reasoning, the single-channel
  sibling of FLP). Leaves 16–24 are all *unscheduled* open-ended
