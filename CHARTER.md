...
inversion; the derivations' *independence* hides the past message keys — preimage resistance alone is
necessary but not sufficient). So the swap is load-bearing in a **weaker** sense than pow's: pow's toy made
...
whether the held key has a unique preimage**. At a *colliding* held key (≥2 preimages) the past is
**information-theoretically** ambiguous — `ratchet_past_ambiguous_at_collision` *proves* no function
recovers it there (a residue *discharged*, bloom's sibling — localized per-key; its global shadow
`ratchet_noninjective_no_past_recovery` only says a non-injective step has no *universal* left inverse); at
a *unique-preimage* held key the past is *determined* but recovering it needs the step's left-inverse
(`ratchet_left_inverse_recovers` exhibits that a left inverse suffices) — a SHA-256 preimage search,
discharged *outside* Lean (a residue *named*, pow's sibling; naming SHA-256 in this branch is a
**conservative worst case** — a random-oracle SHA-256 step is generically *non*-injective, likely the
discharged branch). The first wire whose residue is proved under one backend property and named under the
other. The reduce-half `ratchet_held_reaches_all_future` also makes a prose
