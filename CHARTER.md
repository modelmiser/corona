...
- **Leaf 15:** nothing to promote (standalone, imports nothing — a boundary-drawing leaf
  ...
  **code** (leaf 4: no core module applies) or a swap-placeholder that recurs without
  promoting (leaves 9–13). Leaf 15's is neither: its *second half is discharged by nothing
  at all* — not by a core module, not by a primitive, not by a runtime check, but by a
  **proof that lives in another repo**. So the check sharpens the garden's self-image one
...
state), but *the merge being a semilattice join* (the four laws that make replicas
converge) reduces to no primitive at all; it is an algebraic proof obligation, and the
wrong merge type-checks just as readily as the right one. The `Clone`-vs-linear axis maps
onto CALM's monotone-vs-non-monotone: leaf 9's linear coin needs coordination, leaf 15's
`Clone` counter needs a proof). Corona names the family these already form; it is
...
- *(None scheduled. The **vocabulary is complete** (leaf 6), **composition is
  ...
  **Every named breadth candidate is built, and the two sibling gardens are both wired to
  ...
  leaf 14; a monotone CRDT and the seam to the proof face — done, leaf 15.))*
