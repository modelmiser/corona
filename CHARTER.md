...
- **Leaf 19:** nothing to promote (standalone — imports neither core module nor a sibling leaf;
  ...
  inapplicable**. Every prior residue names a fact about a value (`count` leaf 1/12), a
  *production history* of a value (`cost` leaf 18 — pointedly *not* a fact about the value, as
  leaf 18's own finding stresses), a relation (`order` leaf 17), a timeline (`freshness` leaf
  11), or a domain law (`proof obligation` leaf 15). Leaf 19's residue — **unlinkability** — is
  the first that is a property
