...
| `vdf-types` | research (toy) | verifiable delay function (RSW + Wesolowski) | does **"`T` sequential steps of work elapsed"** reduce? → **it SPLITS, adding a residue of a new kind: a complexity lower bound.** *Validity* reduces to E0451 (`Vdf::verify(output, proof)` is the sole minter of a sealed `Evaluated` via the Wesolowski identity `π^ℓ·x^r ≡ y mod N`, with `ℓ = H(x,y,T)` and `r = 2^T mod ℓ`; `pow`/`merkle`'s `verify` again, and verification is *exponentially cheaper* than evaluation). *The delay does NOT reduce*: the seal witnesses `y = x^(2^T)` and **nothing about how long the producer took** — the same output reached by `T` honest sequential squarings, or in **one** step by a party who knows `φ(N)` (reduce the exponent), mints the **byte-identical** witness, because the delay is not a property of the value; `Vdf::eval` hands the squaring count back as a *return value of the computation*, deliberately not a field of the witness (∥ pow's attempts). This is a **sibling to `pow-types` (leaf 18) on a different axis, and the contrast is the leaf**: pow's residue is *cost* — a fact about a value's **production history** (a lucky first guess is cheap; unconditional); vdf's is a **sequential-depth lower bound** — a fact about what *no* computation can do faster (no lucky shortcut, the output a deterministic function, the bound quantified over *all* algorithms and conditional on the group's order being hidden). It sharpens *the seal witnesses the checked path and nothing more* one more axis: silent about the *math* of the path (leaves 3/4), the *direction* of its soundness (leaf 16), the *history* of reaching it (leaf 18 — cost), and now the **sequential depth** any reaching of it must have. **∥ leaf 6 / leaf 18** the delay *parameter* still reduces — `Vdf<const T>` is walled `1 ≤ T ≤ 63` (E0080; `T=0` = the identity map, a domain invariant, ∥ pow `BITS≥1` / leaf 6 `K≥1`; `T=64` overflows the toy's `u64` proof arithmetic, an **honestly-disclosed toy limit, not** a domain impossibility the way leaf 18's `BITS≤64` is — the two walls' *different* justifications are themselves the nuance), so leaf 20 is the **third E0451 + E0080** leaf; brand/E0382 honestly unused. Standalone. **The toy INVERTS the usual break** (∥ leaf 19): the Wesolowski *verification* is faithful while the tiny `N=3233` factors → `φ(N)` known → the *delay* is broken (executable — a trapdoor shortcut mints the identical witness); a real VDF needs a group of **unknown order**. Proof soundness also rests on a hardness assumption absent in a small group; toy FNV challenge derivation; witness unbranded (`owns`-detectable ∥ leaf 18) |
...
- **Leaf 20:** nothing to promote (standalone — imports neither core module nor a sibling leaf;
  ...
  category** and a *sibling axis* to leaf 18. Every prior residue names a fact about a value
  (`count` leaf 1/12, `cost` leaf 18), a relation (`order` leaf 17), a timeline (`freshness`
  leaf 11), a law (leaf 15's `proof obligation`), the decoder's fate (`emergent completion`
  leaf 13), or the observer's view (`unlinkability` leaf 19). Leaf 20's residue — the
  **sequential delay** — is the first that is a **complexity lower bound**: a fact not about the
  value, its history, or any observer, but about *what no computation can do faster*. The
  contrast with leaf 18 is the finding: pow's *cost* is a fact about a value's **production
  history** (a lucky first guess is cheap; unconditional, a property of *one* search), whereas
  vdf's *delay* is a **lower bound quantified over all algorithms** (no lucky shortcut — the
  output is a deterministic function — and conditional on the group's order being hidden). So it
  sharpens *the seal witnesses the checked path and nothing more* one axis further — silent about
  the *math* (leaves 3/4), the *soundness direction* (leaf 16), the *history* (leaf 18), and now
  the *sequential depth* of any reaching of the path. Meanwhile *validity* reduces to E0451 and,
  ...
  with the toy *inverting* the usual break as leaf 19 did (faithful verification, broken delay —
  the tiny modulus's order is computable, so the sequentiality has a trapdoor).
...
the same output reached by `T` squarings or in one trapdoor step (knowing `φ(N)`) mints the
byte-identical witness. Where leaf 18's *cost* is a fact about *one search's* history
(unconditional, with lucky shortcuts), leaf 20's *delay* is a lower bound over *all* algorithms
(no shortcut, conditional on hidden order) — the seal now also silent about a checked path's
*sequential depth*. ∥ leaf 6 / leaf 18 the delay *parameter* reduces (E0080, `1≤T≤63`), the third
E0451+E0080 leaf; the toy *inverts* the break as leaf 19 did — faithful verification, broken delay
because the tiny modulus's order is computable). Corona
...
- *(None scheduled. The **vocabulary is complete** (leaf 6), **composition is
  ...
  **Every named breadth candidate is built, and the two sibling gardens are both wired to
  ...
  where validity reduces to the seal but the sequential delay does not — done, leaf 20. Leaves
  16–20 are all *unscheduled* open-ended domains seeded after the garden was already a finished
