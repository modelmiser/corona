...
> **⚠ "fan-in 0" and "blast radius" are scoped to the LEAF GRAPH (2026-07-23).** They mean
...
> prose predates** — so the claim was true when written and decayed silently, which is the
> failure mode a scope definition is supposed to prevent.
...
2. Every illustrative backend is replaced by a vetted dependency behind the
   ...
   *(Second caveat, learned graduating `mss-types` 2026-09-08: a **composition** leaf
   owns no backend — its seams are its parents' — so #2 is **inherited**: it holds
   exactly when every parent it composes has graduated, and cannot hold earlier.
   What stays illustrative in a composition (a demo seed, a fixed capacity) is not a
   backend and is disclosed under #3, not swapped under #2. The criterion's own
   text, "every illustrative backend", is then vacuously satisfied — say so in the
   row rather than let the vacuity pass as a swap. **Transitive**, settled by the
   second composition graduation (`hypertree-types` 2026-09-09, whose parent `mss-types`
   had itself graduated by inheritance): a leaf with no backend satisfies #2 however
   deep the nesting, because the criterion quantifies over backends the leaf owns and
   there are none at any depth. What that does NOT license is treating the label as
   earned by the flip — #4 and #5 are the criteria a composition can still fail, and
   they are where its content is.)*
...
> ⚠ **Enforcement note, added 2026-07-23 (cold review round 8), because the list above says
> "only when **all** hold" and the registry does not honour it.**
>
> ⛔ **The first version of this note (round 8) got its premise backwards and is corrected here
> (round 9).** It said #5 was "recorded for exactly one of the ten `**graduated**` rows" and that
> "the other nine carry the label with no recorded assessment against #5 at all". **False**, and
> refuted by the table this note is about: **seven of the ten rows record #5 in the registry
> itself** (`merkle`, `ratchet`, `pow`, `bloom`, `consttime`, `commit`, and `accumulator`), and an
> eighth — `lamport` — carries a 21-round record in `TODO.md` that this very note cites fifteen
> lines below. Only `ecash` and `translog` are unassessed here. The false sentence was doing the
> note's rhetorical work ("the only one visibly failing it"), and it was wrong.
>
> What survives, and it is enough: `accumulator-types`' own record shows **no clean round yet** (the count
> lives in `TODO.md` and is deliberately not duplicated here), so *this* row is labelled `**graduated**` while its #5 is recorded as unmet, against
> a criteria list that says "only when **all** hold". Six sibling rows record #5 as *converged*, so
> the registry is not uniformly unassessed — it is inconsistent, and this row is the inconsistent
> one.
>
> That is a contradiction between this list and the table below, and it is **not resolved here**,
> because resolving it is a judgement about the garden and not about any one leaf. The two
> coherent resolutions are: (a) add a third track state — `swapped, review open` — and demote
> every row whose #5 is unrecorded, or (b) amend #5 to the bar actually applied and say what it
> is. Doing (b) *while a review is open on the leaf being measured* would be changing the gate to
> fit the result, so it is not being done mid-arc.
>
> Two further facts a resolution needs. **#5 is not objectively evaluable as written**: "clean"
> is never defined (the de-facto standard across ~30 arcs in `TODO.md` is 0 CRITICAL + 0
> MODERATE), "cold review" is not defined (lens count and independence have varied 1–4 within a
> single arc), and "the graduated surface" is not bounded — for this leaf it grew twice mid-arc,
> first to CHARTER/README/siblings and then to the checking scripts, and it now spans two
...
post-compromise. (Non-hub — fan-in 0 and fan-out 0 across the leaf graph; imports only `sha2`, which the graduation itself added — an earlier version of this parenthetical said "imports nothing" and was false after its own swap.)
...
the **first hub graduation with zero COMPILE-TIME blast radius**. Merkle's widened its `Digest` (`u64 → [u8; 32]`) and
...
unforgeability properties this construction needs — the toy failed every one, though only two
are supplied usefully — (textbook Lamport needs two; deriving all preimages from a seed incurs a third — `prg` must be **unpredictable under its seed**, not merely one-way — PRF-ness is a *further* assumption that prices the cost table without moving the ~2³² floor) — `commit` one-way and `prg` unpredictable, where the toy failed both **outright** — over a fixed-length input FNV-1a is affine in bounded perturbations, so inversion is a dimension-8 modular knapsack that lattice reduction solves in well under a second per target, completely and memory-free; the toy's cheapest break was total key recovery in seconds, and an intermediate draft's "~2³² meet-in-the-middle" was a wrong correction of a true statement, and
...
exponent** at all: **against a correctly-used key the binding constraint is now the WIDTH, not
the hash**, and the leaf keeps a not-for-production marker (its description would otherwise advertise
stronger than the `mss-types` that composes it). `Sol.Lib.Lamport` (the 4th wire) **moves no PRE-EXISTING theorem** — it
quantifies over an abstract `accepts` and an abstract digest, so no hash appears in any *statement* (the
prose names SHA-256; the theorems do not; Part 3 later adds `Bits := Nat → Bool`). The
correct precedents are **pow and ecash** — and **ratchet**, whose swap is load-bearing in the weaker abstained-guarantee flavour — all load-bearing swaps over abstract-backend wires, *not*
bloom/translog, whose swaps were not load-bearing; lamport's wire **pre-existed its graduation**, so the question "did
the swap move anything?" could be asked of theorems written beforehand — and the answer is no. (No
uniqueness claimed: translog's and commit's wires also pre-existed theirs.) Invariance under a security-critical change is a
statement about the model's **coverage**, not a design triumph — and the swap did not *contribute* Part 3
either: the **coverage lemma** is backend-independent and could have been proved the day the wire was
written, so the graduation **occasioned** the review that noticed it was missing rather than producing it.
Its content: the two-message forgery condition is purely combinatorial (`d₃` is forgeable from
`{d₁, d₂}` iff `d₃` agrees with `d₁` wherever `d₁` and `d₂` agree), so Lean holds it with no hash and no
preimage — though "no oracle" is not a virtue: `held`'s "and no others" *assumes* `commit` one-wayness AND `prg`
unpredictability — the first of which the swap bought, the second being capped by the 64-bit
seed and false outright for the crate's ≲2²⁵ demo seeds — so the lemma's PROVABILITY is
backend-independent while its FAITHFULNESS is not — the one thing the first draft wrongly declared out of model. Residue untouched, as
...
The **tenth**, 2026-07-22/23, is `accumulator-types` (leaf 11, toy FNV-1a → domain-separated
**SHA-256** truncated to `u64`, behind the unchanged `leaf_hash`/`node_hash` seam) — the
**eighth non-hub**, and **type-preserving** (`u64→u64`, ∥ lamport), so zero compile-time blast
radius. It contributed `Sol.Lib.Accumulator`, the **17th wire**. **Read this entry with its
caveat: criterion #5 is NOT yet earned.** The gate is two *consecutive* clean cold-review
rounds and **every round so far has been NOT CLEAN**, so the registry row above marks the backend
swap, not a convergence claim; `TODO.md` carries the round-by-round record and is the referent.
(This paragraph said "rounds 1–4" and "three times running" for three rounds after those
numbers stopped being true; the replacement said "rounds 1–7" in a round that was already the
eighth. **Four wrong values in a row.** The mechanism is not carelessness: a count that changes on
the same cadence as the edits correcting it can never be right at rest, so it is no longer
restated here — `TODO.md`'s `[x] **Round N**` entries are the authoritative value.)
What the arc is *for* is already clear even unconverged: **every round's CRITICAL was
introduced by the previous round's fix**, six times running, at first always in the same
paragraph and later well beyond it —
a summary of `lamport-types`' FNV analysis that four separate drafts got wrong (a paraphrase,
an arithmetically false affine claim, a re-derivation shipped as "verbatim", then a correct
restatement shipped as "quoted exactly" with the source's `fixed-length` qualifier dropped).
The resolution was **subtraction**: the file no longer restates the analysis at all, it cites
it, and keeps only what it can test. *A summary of someone else's argument is a claim with no
checker on it.* Round 4 added the structural counterpart — the sweep for a retired phrase must
run against a tree that matches HEAD, because a sweep over a mutated tree does not error, it
silently reports success.

The **eleventh**, 2026-09-08, is `mss-types` (leaf 7) — the **first composition graduation**,
and the first with **nothing to swap**: a composition's seams are its parents' (criterion #2's
second caveat, above), both of which graduated in July, so #2 holds by inheritance and the
graduation changes no non-test code and no value — the **ninth non-hub** by fan-out (`hypertree-types`
...
The **twelfth**, 2026-09-09, is `hypertree-types` (leaf 14) — the **second composition
graduation** and the first by **transitive** inheritance, its parent `mss-types` having itself
graduated by inheriting; #2 is vacuous at any depth (the second #2 caveat, above), a **sink** in
the dependency graph so the blast radius is zero of every kind, and no code or value moved. It
contributed `Sol.Lib.Hypertree`, the **19th wire**, whose two results have opposite sign. The
crate's headline finding — two coordinated linear counters — **collapses** on the proof face: the
odometer is one counter in mixed radix, so its safety facts are wire 18's under a change of
representation, and the coordination is real at the *type* level only. Its *bonus* finding is the
one that pays: leaf 7's caller-trusted `adopt` anchor, which wire 18 could only **record** as
unpinned, becomes **pinned** here, so this is **the first residue edge the garden has closed by
composing rather than by naming** — conditioned on an injective anchor digest, which the inherited
64-bit width prices at ~2³², and that condition corrected the crate's own unqualified prose. The
persistence boundary (a restored state value signs twice at one index) stays open and is why the
stateless SPHINCS+ exists. Criterion #5 is its own arc; `TODO.md` carries the round record.
...
| `lamport-types` | **graduated** | Lamport one-time signatures (hash-based) | the first leaf whose **central primitive is not the E0451 seal** — it centers **E0382 move-linearity**. A one-time signing key is a *consumable capability*: `SigningKey::sign` takes `self` **by value** (and the key is not `Clone`/`Copy`), so signing twice does not compile (verified: `error[E0382]: use of moved value`). → **the one-time-use invariant reduces to E0382, no new primitive.** Sharpens the *evidence-of-a-fact* (Clone-able sealed witness, E0451 — leaves 1–4) vs *consumable-capability* (linear value, E0382 — this leaf) distinction, in crypto. Honest nuance: Rust moves are **affine** (at-most-once), not full **linear** (exactly-once) — which is *exactly* OTS's need (double-sign is the catastrophe; not-signing is safe). Still keeps an E0451 seal (`VerifiedMessage` from `verify`); redacting `Debug` on the secret key (∥ Shamir `Secret`). Imports nothing from `corona-core` (∥ merkle). **GRADUATED 2026-07-22** (9th graduation, and the **SECOND HUB** after merkle — fan-out onto `mss-types` and, transitively, `hypertree-types`, the hash-based-signature track): toy FNV-1a → vetted **SHA-256** (u64-truncated) behind the unchanged `digest`/`commit`/`prg` seam (criterion #2). **The first hub graduation with zero COMPILE-TIME blast radius** — the swap is type-preserving (`u64 → u64`), where merkle's `u64 → [u8; 32]` forced edits in every dependent; only hash *values* moved, hence the breaking `0.1.0 → 0.2.0` bump. **LOAD-BEARING (∥ pow, ecash) on ALL THREE unforgeability properties this construction needs — the toy failed every one — though only two are supplied at a useful level, and the graduation's own cold review is what established the count.** Textbook Lamport needs `commit` one-way AND `digest` collision-resistant; deriving all 128 preimages from a seed incurs a third — `prg` must be **unpredictable under its seed**, not merely one-way (PRF-ness is a further assumption that only prices the cost table), since a predictable `prg` hands over every unrevealed preimage from a single signature. The toy failed all three — `commit` and `prg` **outright**, and `digest` too (same-length collisions fall out of the same enumeration) (FNV-1a over a fixed-length input is a lattice-solvable dimension-8 knapsack — inverted in well under a second per target, so the toy's cheapest break was total key recovery in seconds; an intermediate review draft's "~2³² meet-in-the-middle" was itself a wrong correction, walking back a true claim); SHA-256 supplies one-wayness at ~2⁶³, giving the scheme its first non-trivial exponent. But `verify` re-derives `digest(message)` and checks preimages against *that*, so a signature binds to the **digest**: at the illustrative 64-bit width a birthday collision forges at **~2³²** under chosen message — demonstrated offline (~2³² evaluations) and now executable in-crate (`a_digest_collision_forges_across_keys_at_the_toy_width`, key-independent). So the swap upgrades the *class* of break (universal-forgery-from-the-public-key → existential-forgery-needing-a-signed-message) without making the scheme unforgeable: **the binding constraint is now the WIDTH, not the hash** — though only for a key used *correctly* (uniform seed, discarded; at most one signature). This crate's own examples violate both: a low-entropy literal seed falls in ≲2²⁵, and a retained seed lets its holder re-mint and sign freely (the two-signature harvest it enables costs ~2⁹–2¹⁰ hash evaluations (a unit that understates its total work — the route then scans ~2^26.6 triples) for a chosen-message adversary choosing all three messages jointly — the harvest is a *weaker* attack than the hole reaching it). So the leaf keeps a not-for-production marker, and "the cheapest break is ~2³²" is a claim about correct usage, not about the demonstration. The Sol wire `Sol.Lib.Lamport` (the 4th wire) is **hash-agnostic by construction** — digest an abstract `Nat`, check an abstract predicate — so the swap **moves no pre-existing theorem** (the right precedents are **pow, ecash and ratchet**, load-bearing swaps over abstract-backend wires — *not* the non-load-bearing bloom/translog; lamport's wire pre-existed its graduation, so the swap was testable against theorems written beforehand — no uniqueness claimed, translog's and commit's did too); and Part 3's *coverage lemma* plus the thin `signature_transfers_along_digest_equality` (the ~2³² break; separate because it is about `accepts` rather than `forgeable`, not because of the message layer it carries — `Msg` is eliminable from it) were OCCASIONED by the graduation's review rather than contributed by the swap, being backend-independent. Residue untouched: E0382 move-linearity (the seed re-mints the key), which no hash strengthens. Downstream `mss-types`/`hypertree-types` inherit the graduated backend at **both** layers (Merkle already SHA-256), closing the "toy via Lamport" caveat — they stay research-rung via *composition* (deterministic seeds, fixed capacity), not via hash |
...
| `mss-types` | **graduated** | Merkle Signature Scheme (many-time signatures) | the first **composition leaf** — do leaves compose through **public surfaces only**, with no new primitive? → **yes.** MSS (Merkle 1979) = `merkle-types` ∘ `lamport-types`: a hash tree over *n* one-time verifying keys, root = one many-time public key. Three primitives jointly, each doing its home job: **E0382 lifted** from key to keychain (`sign_next(self, …)` consumes the chain state → stateful-signature stale-state reuse *of a chain value* is a compile error, verified `error[E0382]` — conditional on seed discard, as leaf 5 disclosed: a retained seed re-mints the chain; each inner `SigningKey` consumed by leaf 5's own `sign`); **E0451 conjoined** (sealed `VerifiedMssMessage` minted only when *both* leaves' sole minters fire — Lamport verify AND Merkle membership); **brand penning the intermediate** (`VerifiedLeaf` born and dead inside `adopt_scoped`; only unbranded facts escape — its anchor-relative index, joined with the digest and the key's `(root_hash, capacity)` anchor). E0080 honestly unused (3 of 4). **The composition finding:** it demanded two additive rungs on the composed leaves — `merkle_types::adopt_scoped` (verifier-side/light-client root adoption; leaf 4 was committer-complete but verifier-scope-bound) and `lamport_types::VerifyingKey::to_bytes` (canonical key identity to commit to) — both ordinary API in the existing vocabulary, no private access: **composition pressure surfaces missing API, not missing vocabulary.** Cold review then showed the pressure *propagates up*: leaf 7 initially re-created both component gaps one level higher — a provenance-less composed witness (vss/merkle's rung-1 provenance gap) and a verifier-unconstructible public key (leaf 4's pre-adoption gap) — closed by full-anchor value provenance (`VerifiedMssMessage::minted_by`, recording `(root_hash, capacity)` — round 2 showed the hash half alone can't tell an honest key from a same-hash lying-capacity adoption, whose overstated capacity accepts genuine material at phantom `key_index`es; membership never degrades, position semantics do; round 3 added that adopted anchors also inherit merkle's duplicate-leaf orbit — a degenerate caller-built anchor lets one signature verify at each duplicated `key_index`, all honestly `minted_by` the same anchor — disclosed + regression-tested) and `MssPublicKey::adopt`; a brand would scope the deliberately-distributable key. *A composition inherits its components' obligations, not just their guarantees.* Inherits lamport's seed caveat (chain-*value* linearity, hash-independent — a retained seed re-mints the chain); its hash layers are **both graduated SHA-256** now (Merkle from leaf 4, Lamport from leaf 5's 2026-07-22 graduation), so until 2026-09-08 it stayed research-rung via *composition* (deterministic seeds, fixed capacity), **not** via a toy hash. **GRADUATED 2026-09-08** (11th graduation, the **FIRST COMPOSITION graduation**, 9th non-hub — fan-out onto `hypertree-types` only, and since the change is documentation and tests the blast radius is zero of every kind, not merely compile-time): criterion #2 **inherited** (no backend of its own; both parents graduated in July — the second #2 caveat), #3 the Honest-limits section unchanged in substance, #4 `Sol.Lib.Mss`, the **18th wire** and the first for a composition — **zero rungs on either parent's wire**: the keychain is Lamport's two-state capability lifted to a cursor (`emits_are_consecutive`: a run emits exactly the consecutive indices from the cursor, hence `no_key_index_is_emitted_twice` and `at_most_capacity_signatures`), and the composed acceptance pins the presented key to the committed one by `Merkle.fold_pins_leaf_and_path` as shipped (`accepted_key_is_the_committed_key` — a forgery of the composition is a forgery of a component or a hash collision, **along one index, under an honest anchor**; both qualifiers load-bearing, since a capacity lie through `adopt` is a different anchor that reassigns committed bytes to another index with no collision — the crate's two capacity-lie tests have no image in a model without `capacity`); the residues are restated in the same motion (`signature_transfers_along_digest_equality` — the ~2³² width in Lamport's Part 3 shape, generic in the digest type and not derived from the parent; `collision_breaks_key_binding` — stated at the composed acceptance on a two-leaf tree, so the headline's `hcf` is load-bearing at the theorem; `accepts_under_some_anchor` — the caller-trusted anchor recorded by a trivially true existential, not evidenced; `restored_chain_defeats_linearity` — the seed caveat as a proved contrast). Part 1 and Part 2 are joined by prose, not a theorem: an index-carrying model would have needed an `authProof`-totality rung Merkle lacks, which is the qualifier on "zero rungs". Written the day of the graduation decision, so "the graduation moved no theorem" is a counterfactual, not an observation (∥ accumulator). All eleven results are within Lean's standard three axioms; the transfer carries `propext` from `foldUp`'s definition (its Lamport parent mentions no fold — the carrier is the statement, not the tactic). **The datum, qualified:** the thesis held on the proof face for what the model states. Criterion #5 — the re-review of the graduation text is its own arc (garden-cold-review workflow, three blind lenses, plus a cross-vendor pass on the public claims); the round-by-round record lives in `TODO.md`, the referent, and is deliberately not restated here (the accumulator lesson: a count that changes on the cadence of the edits correcting it is never right at rest) |
...
| `hypertree-types` | **graduated** | XMSS^MT-style hypertree signature (`mss ∘ mss`) | the garden's first **recursive** composition — a top `mss` keychain signs the *root of a bottom `mss` keychain*, the bottom signs the message, so one long-term key certifies a `top×bottom` virtual keyspace (subtrees regenerated from a seed). Does composition **nest**? → **yes, with no new primitive** and (∥ leaf 8) **zero new rungs** into leaf 7 — built entirely on `mss-types`' public surface (`generate`/`sign_next`/`adopt`/`verify`/`minted_by`), reused verbatim. Three connected findings: **(1)** composition is not just *repeatable* (leaf 8) but *self-nesting* — a leaf composed with itself. **(2) THE NEW DATUM — composing *two* stateful leaves needs *coordinated* linear state.** Leaf 8 composed two *stateless verifications* (erasure/merkle verify are pure); leaf 7 composed *one* stateful operand (lamport's linear key, E0382) with stateless merkle — a *single* linear counter ("E0382 lifted from key to keychain"). Leaf 14 is the first to compose **two** stateful operands: both `MssKeychain`s carry a linear counter, and `HyperKeychain::sign_next(self)` threads **two** *in lockstep* (bottom per-signature, top per-subtree-exhaustion) inside one move — the whole nested state is one linear object, so a stale hypertree is a compile error (E0382, verified) and no counter can desync. The new datum is the *coordination of two* counters (leaf 7 already had one), **not** statefulness per se; E0382 is exactly the tool. **(3)** the real-world catastrophe (one-time-index reuse) lives at the **persistence boundary** — E0382 guards the *in-memory* state, never a serialized/restored/VM-cloned copy; this is leaf 9's *wire boundary* + leaf 11's *unbranded-wire* finding, now for **signature state**, and precisely *why stateless SPHINCS+ exists* (made executable 2026-07-19 by `the_persistence_boundary_reuses_a_one_time_index_across_a_restore`: two seed-restored copies sign different messages at the same one-time index). **(bonus)** composition can **discharge** an obligation, not only inherit one (∥ leaf 7's inversion): leaf 7's `adopt` capacity-lie is *closed* here because the top **signs** the child's full `(root, capacity)` bytes — the adopted subtree key is authenticated, not caller-trusted (regression-tested). Sealed `VerifiedHypertreeMessage` (E0451) minted only when *both* links verify (four sole-minters two levels deep). E0382 + E0451, brand inherited internally (mss's), E0080 unused; **no new primitive**. Imports one leaf (`mss-types`) — the *self-nesting* dependency, emphasizing `mss ∘ mss`. Both hash layers are now **graduated SHA-256** (Merkle from leaf 4, Lamport from leaf 5's 2026-07-22 graduation, inherited via `mss-types`); what remains illustrative is the *composition* — deterministic seeds, 2 fixed layers, no state-persistence protocol — so it stays research-rung, not via a toy hash  **GRADUATED 2026-09-09** (12th graduation, the SECOND COMPOSITION graduation and the first by TRANSITIVE inheritance — its parent `mss-types` had itself graduated by inheriting, and #2 is vacuous at any depth for a leaf that owns no backend; a SINK in the dependency graph, so zero blast radius of every kind, and no non-test code or value moved): #2 inherited transitively (the second #2 caveat), #3 the Honest-limits section extended by the wire's collision-scope finding, #4 `Sol.Lib.Hypertree` — the **19th wire**, second for a composition — whose two results have OPPOSITE SIGN. **The crate's headline finding COLLAPSES:** the odometer is one counter in mixed radix (`signNext_increments_flat` — a carry is an increment — then `emits_flatten_to_a_run`), so `no_index_pair_is_emitted_twice` and `at_most_the_product` are wire 18's single-cursor facts under a change of representation; the two-counter coordination is real at the *type* level (one `self`-by-value move advances both) and a re-encoding at the proof level. **The bonus finding is the real one:** `Sol.Lib.Mss.accepts_under_some_anchor` could only RECORD that nothing pins the subtree anchor, and composing `mss` with itself PINS it (`capacity_lie_needs_an_anchor_collision`) — **the first residue edge the garden has closed by composing rather than by naming** — with its own new residue stated, not hidden: the discharge holds only up to an injective anchor digest (`discharge_fails_without_injectivity`), which the inherited 64-bit width prices at ~2³², so the capacity lie moves from free to dear and not to impossible. That condition CORRECTED this crate's own prose, which had claimed the lie simply fails. The persistence boundary stays open (`restored_hypertree_defeats_linearity`): a restored state value signs twice at one index, E0382 guards the in-memory value only, and no primitive in the vocabulary closes it — the standing argument for the stateless SPHINCS+. Four of ten results axiom-free. #5 is its own arc; `TODO.md` carries the round record and neither this row nor the narrative restates a count |
...
| `accumulator-types` | **graduated** | append-only Merkle accumulator (witness staleness) | the first leaf to point the **E0308-class brand** at *time* not *provenance* — an accumulator evolves (`add` advances the epoch), so a membership witness goes **stale**; does "fresh against the current accumulator" reduce? → **it SPLITS** (∥ leaf 9's double-spend, but drawn *inside the brand* ∥ leaf 10 inside E0382). **Snapshot-identity binding reduces to the brand**: each immutable snapshot is frozen in a fresh generative-lifetime scope (`snapshot_scoped`); a `Commit<'epoch>` and the sealed `Included<'epoch>` it mints share the brand → cross-snapshot use is a compile error (verified: `lifetime may not live long enough` + E0521, the vss/merkle signature — merkle rung-2's mechanism on evolving ground). **Freshness itself does NOT reduce** — a runtime check: a `Witness` crosses the wire so (∥ merkle `Proof`) it is **unbranded by necessity** (can't brand serialized bytes); with no brand, staleness = comparing epoch *numbers* at runtime (`VerifyError::Stale`), the same runtime residue as leaf 9's redeem-time freshness (leaf 1's share-counting is a DIFFERENT residue — a count, not a freshness test, as the crate itself is careful to say). **NEW DATUM = the boundary is INSIDE the brand**: the brand captures snapshot-*instance* identity (value-level) but structurally CANNOT capture epoch *freshness* (timeline) — a brand is fixed at creation, advancing mints a *new* snapshot not a re-stamp. Two executable consequences: (1) two snapshots at the SAME epoch get DIFFERENT brands (compile-fail doctest → brand is finer than the epoch number, and unordered); (2) the verified *result* (`Included`) carries the brand, the incoming *request* (`Witness`) can't → the brand guards the answer's provenance, never the question's freshness, and the wire is where the reduction stops. Two primitives (E0451 + brand), no new one. Standalone (∥ merkle/lamport/ecash/ratchet — reuses merkle's brand *discipline*, not its *code*; importing merkle's per-adoption brand would tangle it with this leaf's per-epoch one). Append-only (epoch == count, so **within one accumulator** epoch-staleness implies root-staleness — the converse fails, since `Accumulator` is `Clone` and two forks reach one epoch with different roots; the explicit epoch check makes staleness a named/total/hash-independent verdict carrying no security weight); no deletion/consistency-proofs/compaction (MMR/CT territory) **GRADUATED 2026-07-22** (10th graduation, 8th non-hub): toy FNV-1a → domain-separated **SHA-256** (`sha2`) truncated to `u64`, behind the unchanged `leaf_hash`/`node_hash` seam (criterion #2). **Type-preserving** (`u64→u64`, ∥ lamport, unlike merkle's `u64→[u8;32]`), so **zero COMPILE-TIME blast radius** — but *not* zero dependents: `tools/compose-probes` path-depends on this leaf, and its `Cargo.lock` needed the version bump. An earlier version of this row said "zero dependents, so no blast radius at all"; the conclusion held and the premise did not, which is the more dangerous way to be right. Values move, hence `0.1.0→0.2.0`. The swap removes the toy's OUTRIGHT break (FNV-1a inverts by lattice reduction over a small modular knapsack — affine in *bounded perturbations*, not in the bytes), so the ACHIEVED bound really did move, from effectively zero bits to 32. What it cannot do is raise the **CEILING the width imposes**: a root binds only as well as the hash resists **collisions**, and a 64-bit seam yields a **~2³² birthday** bound offline and key-independently (memory-free via van Oorschot–Wiener distinguished points at essentially the same cost — the "~3× memory-free" this row carried until 2026-07-23 was Floyd cycle detection's price, and over-priced the attacker); a *fixed* target is dearer at ~2⁶⁴ second-preimage — **the ceiling is the WIDTH, not the backend** (∥ lamport's ~2³², same cause: a `u64` seam truncates whatever fills it). Keeps its not-for-production marker. Lean `Sol.Lib.Accumulator` contributed (#4) — the brand's INTRA-PRIMITIVE boundary — but ⚠ **NEITHER direction is proved as an impossibility, and an earlier version of this row said "as theorems, both directions"**: instance-identity is FINER than epoch identity (`same_epoch_distinct_scopes` — an **existential**, and both the Lean file and scoreboard row 71 carry the caveat this row dropped for a round: it *records* generativity, it does not *evidence* it, since the ∃ form holds for any structure with a two-inhabitant tag field) and a stale witness is rejected by EVERY fold, hash-independently (`stale_is_rejected_by_every_fold`) — which is a theorem about the **epoch gate**, not about the brand, and cannot be about the brand since the model's `Witness` has no scope field (that brandlessness is a modelling stipulation the wire's own header declines to prove). Contrast `pow`'s `no_effort_recovery` and `ecash`'s `no_authenticity_recovery`, genuine `∀ f, ∃ …` impossibilities (an earlier version of this row named `ecash`'s `freshness_not_compile_time`, whose conclusion is `False`, not `∃` — the wrong one of that file's two): this wire ships no such theorem, so **this leaf's residue is asserted here and proved next door**; **2 of 7** results axiom-free — the axiom table names which two, and what is deliberately withheld is a *reading of the pattern*, not the identities (an earlier version of this row said no reading is offered "of which", which misdescribes what is withheld; an earlier one still claimed three, and named a theorem that no longer exists). The wire is backend-INDEPENDENT (quantifies over an abstract `fold`), so it moves no theorem — coverage, not triumph (∥ pow/ecash/ratchet — NOT lamport, whose wire pre-existed its graduation by a day and could therefore be asked whether the swap moved anything; this one was written fourteen minutes after the swap, so `Sol.Lib.Accumulator` explicitly disclaims the lamport parallel). Cold review (#5) **CONVERGED 2026-07-23** — rounds 18+19 clean on frozen text (a 19-round arc; both halves — this crate and the `Sol.Lib.Accumulator` wire — clean under lenses hardened against two intervening false-convergences: a scoreboard-row-vs-docstring over-read, and a docstring claiming a test omits an `assert_ne!` the body carried). All five criteria hold. |
