...
- **Leaf 19:** nothing to promote (standalone — imports neither core module nor a sibling leaf;
  ...
  inapplicable**. Every prior residue names a fact about a value (`cost` leaf 18, `count` leaf
  1/12), a relation (`order` leaf 17), a timeline (`freshness` leaf 11), or a domain law (`proof
  obligation` leaf 15). Leaf 19's residue — **unlinkability** — is the first that is a property
...
- *(None scheduled. The **vocabulary is complete** (leaf 6), **composition is
  ...
  **Every named breadth candidate is built, and the two sibling gardens are both wired to
  ...
  unlinkability is a non-relation no brand can hold — done, leaf 19. Leaves 16–19 are
  all *unscheduled* open-ended domains seeded after the garden was already a finished
