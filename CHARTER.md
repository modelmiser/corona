...
preimage — though "no oracle" is not a virtue: `held`'s "and no others" *assumes* `commit` is
one-way, which is exactly what the swap bought, so the lemma's PROVABILITY is
