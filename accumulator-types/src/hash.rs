//! Toy hash backend for the accumulator's Merkle tree.
//!
...
//! It is the same construction `merkle-types` uses — a toy backend is a
//! graduation-swap placeholder, so its textual recurrence across leaves is not a
//! promotion trigger (unlike the permanent `gf256`, which *did* graduate to
//! `corona-core`).
//!
