...
then `consttime-types` the same day (contributing `Sol.Lib.ConstantTime`, the first wire
to formalize a residue's *un-typability* rather than a leaf's invariant) — is
one-directional: a graduated Corona leaf contributes a Lean formalization to Sol.
