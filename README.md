...
[`CWE-MAP.md`](CWE-MAP.md) is the outward search: four CWE pillars (lifecycle, control
flow, protection mechanism, exceptional conditions) mapped against the leaves, with the candidate
gaps outside the garden's current domains.
...
> two-signature harvest costs ~2⁹–2¹⁰ hash evaluations — a unit that understates its total
> work, see `hash.rs` — for a chosen-message adversary who
> picks all three messages jointly, but the re-mint that reaches it costs almost
> nothing). That cap is
...
brand** at *time* rather than *provenance*. Leaves 2, 4, 7 and 8 used the brand to bind
...
- **Snapshot-identity binding reduces to the brand.** Each immutable snapshot is frozen
  ...
  (verified **by direct rustc**: `lifetime may not live long enough` + E0521, the
  vss/merkle signature — the crate's `compile_fail,E0521` doctest proves only that the
  code fails to compile, since rustdoc ignores the error code on stable, the garden-wide
  datum leaf 27 surfaced). It is `merkle-types`' rung-2 mechanism on evolving ground.
- **Freshness itself does *not* reduce** — it is an irreducible **runtime** check. A
  ...
  leaf 1's share-counting stays runtime for a DIFFERENT reason — a count is not a freshness test, and the crate draws that distinction on purpose.
